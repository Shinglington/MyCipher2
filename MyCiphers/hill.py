...
class Hill(Cipher):
	...
	def decrypt(self, ciphertext, keep_punct = False):
		...
		assert len(text) % self.size == 0, "Ciphertext must be a multiple of the matrix size"
		plaintext = ""
		for i in range(0, len(text), self.size):
	...
	def make_matrix(self, key):
		...
		for i in range(self.size):
			matrix.append([self.char_to_int(x) for x in key[i*self.size:i*self.size+self.size]])
		return matrix
